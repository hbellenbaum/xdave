...
def inverse_electron_screening_length(ne, Te):
    ...
    fermi_integral = fermi_integral(
        phi=(chem_pot * beta), j=-0.5, normalise=False
    )

    EF = 0.5 * DIRAC_CONSTANT_SQR * np.cbrt(3.0 * PI_SQR * ne) ** 2 / ELECTRON_MASS

    kappa_sqr = 12 * PI ** (5/2) * ELEMENTARY_CHARGE_SQR / (4 * PI * ELECTRIC_CONSTANT) * ne * beta * fermi_integral / (beta * EF) ** 1.5
...
def ipd_ecker_kroell(Zi, ne, ni, Te, Ti):
    ...
    C = (
        2.2
        * np.sqrt(ELEMENTARY_CHARGE**2 / (BOLTZMANN_CONSTANT * Te))
        * n_c ** (1 / 6)
    )
