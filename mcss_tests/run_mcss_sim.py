...
def run_ch_ar_mode3(
    ...
    params = MCSSParameters(
        ...
        mass_density=rho,
        temperature=T,
        Zfs=np.array([ZH, ZC1, ZC2]),
        ANs=np.array([1, 6, 6]),
        fracs=np.array([xH, xC1, xC2]),
        probe_energy=Eb,
        ipd=user_defined_ipd,
        ipd_model="USER_DEFINED",
        input_lfc=user_defined_lfc,
    ...
    k, WR, f1, f2, f3, q1, q2, q3, S11, S13, S12, S22, S23, S33 = xrts_code_single_ar3(params=params)
...
def run_c_ar_mode(T, rho, Z, angle, user_defined_ipd=0.0, user_defined_lfc=0.0, lfc_model="NONE", plot=False):
    ...
    results_dir = os.path.join(THIS_DIR, f"c_ar_runs_T={T:.2f}_rho={rho:.2f}")
    if not os.path.exists(results_dir):
        os.mkdir(results_dir)
    ...
    params = MCSSParameters(
        ...
        mcss_executable=mcss_executable,
        mcss_dir=mcss_dir,
        deck_file=f"mcss_ar_run_c_T={T:.2f}_rho={rho:.2f}_Z={Z}_model={lfc_model}",
        output_file=os.path.join(results_dir, f"mcss_ar_run_c_T={T:.2f}_rho={rho:.2f}_Z={Z}_model={lfc_model}"),
        mass_density=rho,
        temperature=T,
        Zfs=np.array([Z_min, Z_max]),
        ANs=np.array([6, 6]),
        fracs=np.array([x1, x2]),
        probe_energy=Eb,
        ipd=user_defined_ipd,
        ipd_model="USER_DEFINED",
        input_lfc=user_defined_lfc,
